...
function vaciarCarritoConTotal() {
    ...
    return console.log( carrito, total);
}
...
function cuentaDeAlumnos() {
    ...
    grupoUnico = grupoA.concat(grupoB);

    return console.log( grupoUnico.length );
}
...
function transmitiendoBatch() {
    ...
    longSecciones = Math.ceil(backup.length/4);

    secciones=[];

    for(i=0; i< backup.length; i+=longSecciones){
        secciones.push(backup.slice(i, i + longSecciones));
    }

    return console.log( secciones);
